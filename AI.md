...
## 🔮 将来の構想：「爆釣指数」のAI予測
このプロジェクトの最終ゴールは、収集したデータを使って**「爆釣指数（仮）」**を算出することや。

- **手法**: Python (Scikit-learn, XGBoost, TensorFlow等) を用いた機械学習。
- **特徴量**: 
  - 気温、水温、潮汐（満潮/干潮、潮位差）、天気、風況。
  - 過去の同時期の釣果実績（魚種別）。
- **目的変数**: 魚種ごとの期待キャッチ数（爆釣指数）。
- **データ連携**: 現在取得している詳細な環境データ（`details` フィールド）は、すべて機械学習の特徴量として使うために設計・収集してるで。

今後、Python環境の構築やデータクリーニング、学習モデルの実装を行う際も、この `AI.md` に知見を溜めていくこと！
